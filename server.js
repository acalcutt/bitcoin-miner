    ...
    socket.on('message', function (msg) {
      ...
      if (text == "getwork")
        if (validJobs.length > 0)
            socket.emit('work', validJobs[validJobs.length - 1]);
      });
...
stratum.on('mining.notify', function(message) {
	...
	log.info('Received jobId '+work.jobId);
...
var appserver = app.listen(process.env.PORT || 3000, process.env.IP || "0.0.0.0", function(){
  var addr = appserver.address();
  log.info("min.lc server listening at", addr.address + ":" + addr.port);
});
